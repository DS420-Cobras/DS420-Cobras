...
#Trying to convert our smape function to a keras-friendly format. Until then, the keras model loss is MAPE.
def ksmape(actual, predicted):
    dividend= K.abs(np.array(actual) - np.array(predicted))
    denominator = np.array(actual) + np.array(predicted)
    
    return (2 * K.mean(K.division(dividend, denominator, out=K.zeros_like(dividend), where=denominator!=0)))
...
def doAnalysis2(cityBej = True):
    ...
    bejDf.fillna(method='bfill', inplace=True)
    ...
    for target in targets:
        ...
        for train_index, test_index in kf.split(df):
            ...
            if algoToUse == 'Keras':
                ...
                lm.fit(X_train,Y_train.values.ravel(), nb_epoch=20, batch_size=5)
            else:
